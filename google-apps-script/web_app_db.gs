...
function doGet(e) {
  ...
  try {
    if (action === 'health') {
      ensureHeaders(getDemoSheet(), DEMO_HEADERS);
      return jsonOutput({ ok: true, storage: 'google-sheets', message: 'Ready' });
    }

    if (action === 'demos') {
      return jsonOutput({ ok: true, items: getDemoRows() });
    }
...
function doPost(e) {
  try {
    ...
    if (body.action === 'saveDemo') {
      ...
      return jsonOutput({ ok: true, id: id, storage: 'google-sheets' });
    }

    if (body.action === 'deleteDemo') {
      deleteDemoRecord(body.id);
      return jsonOutput({ ok: true, storage: 'google-sheets' });
    }
...
function getDemoRows() {
  ...
  return sheet
    .getRange(2, 1, lastRow - 1, DEMO_HEADERS.length)
    .getValues()
    .map(toDemoObject)
...
function deleteDemoRecord(id) {
  if (!id) throw new Error('Demo id is required.');
